...
- `packages/site-gmail/src/background.ts` — service worker (port-based messaging, orchestrator integration via start()/setFilterConfig()/wakeOrchestrator(), selectionChanged handler with cache query + Gmail navigation, cacheState push to sidepanel, cache-complete re-query, return-to-inbox on disconnect, user-navigation detection for auto-tab-switch)
- `packages/site-gmail/src/sidepanel.ts` — Gmail side panel UI (tabs, labels, zoom, pin mode, display settings, label counts, scope-based zero-count label hiding, sends selectionChanged intent to service worker, receives labelResult/cacheState/countsReady)
- `packages/site-gmail/src/gmail-api.ts` — Gmail API client (OAuth2 auth, label fetch, label-query message ID fetch with beforeDate support for gap-fill, per-page fetch variants for orchestrator pagination, scope-based message search, search query builder)
...
- `packages/site-gmail/src/cache-manager.ts` — cache orchestrator (single-loop architecture with decide()/executeAction(), setFilterConfig() for priority control, start(accountPath) with account setup + readiness gate + processed label detection, queryLabel API with co-label counts, getLabelCounts for own/inclusive per-label counts with zero-count omission when scope is active, scopedLabelIdx for pre-computed filtered indexes, progress reporting, incremental refresh, progressive cache depth with scoped initial build, gap-fill for widening scope, background depth expansion)
