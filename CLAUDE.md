...
- `packages/site-gmail/src/summary-deals.ts` — Deals sub-view (finds the `ads/deal` label, fetches its cached message IDs, fetches metadata via Gmail API, renders a sender/subject/date table; click navigates the Gmail tab to the message)
- `packages/site-gmail/src/summary-reminders.ts` — Reminders sub-view (intersects `notifications/calendar` and `pending`, strips `Notification:` prefix and date-range tail from subjects, groups by trimmed subject with `(+N)` count and latest date, click opens a Gmail search of the group)
- `packages/site-gmail/src/gmail-api.ts` — Gmail API client (OAuth2 auth, label fetch, per-page fetch variants for orchestrator pagination, scope-based message search with beforeDate support for parallel segments, search query builder, fetchMessagesMetadata for batched per-message metadata reads)
