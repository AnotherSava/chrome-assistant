...
- `packages/site-gmail/src/background.ts` — service worker (port-based messaging, orchestrator integration via start()/setFilterConfig(), selectionChanged handler with Gmail navigation + setFilterConfig, result callback relay as filterResults to sidepanel, cacheState push to sidepanel, resetCache handler, return-to-inbox on disconnect, user-navigation detection for auto-tab-switch, chrome.storage.onChanged listener for shared settings, getLabelMessageIds resolves a label NAME (not ID) to its label ID + cached message IDs against the live label list, queuing the request until that label's index is ready (cacheManager.isLabelReady) and flushing on each progress event — so the Summary tab never depends on the label-list push, can't get stuck on "Loading labels…", and won't show a premature empty result during a cold build, openMessage navigation handler, gmailHashChanged broadcast on URL/tab changes for Summary row highlighting, fetchError broadcast to all ports on orchestrator start failure with installType-aware hint generation via buildFetchErrorHint)
