...
- `packages/core/src/types.ts` — shared TypeScript types (PinMode, GmailLabel, CacheMessage, CacheProgress)
...
- `packages/site-gmail/src/gmail-api.ts` — Gmail API client (OAuth2 auth, label fetch, label-query message ID fetch, scope-based message search, search query builder)
