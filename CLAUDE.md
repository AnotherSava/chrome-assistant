...
- `packages/site-gmail/src/gmail-api.ts` — Gmail API client (OAuth2 auth, label fetch, per-page fetch variants for orchestrator pagination, scope-based message search with beforeDate support for parallel segments, search query builder, fetchMessagesMetadata for batched per-message metadata reads, fetchMessagesFull for full-body reads with DOMParser-based HTML-to-text extraction that prefers text/html over text/plain)
